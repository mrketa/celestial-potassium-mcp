...
local previous = _G.PotassiumMcp
...
_G.PotassiumMcpGeneration = (_G.PotassiumMcpGeneration or 0) + 1
local generation = _G.PotassiumMcpGeneration
local state = {
...
local function boundedTraversal(root, maxVisited, visit)
	...
	while head <= #queue and visited < maxVisited do
		...
		visited = visited + 1
		visit(node)
		if #queue >= maxVisited then
			if #node:GetChildren() > 0 then
				truncated = true
			end
		else
			for _, child in ipairs(sortedChildren(node)) do
				if #queue >= maxVisited then
					truncated = true
					break
				end
...
function handlers.list_children(params)
	...
	for index = 1, math.min(#children, limit) do
		output[index] = instanceSummary(children[index])
	end
	return {
...
local function boundedInstanceSnapshot(instance, properties, includeAttributes, includeTags)
	...
	for index, child in ipairs(sortedChildren(instance)) do
		if index > MAX_SNAPSHOT_CHILDREN then
			break
		end
	...
	return {
		...
		childrenTruncated = #instance:GetChildren() > MAX_SNAPSHOT_CHILDREN,
	}
end
...
function handlers.snapshot_diff(params)
	...
	local function capture()
		...
		while head <= #queue and #output < maxVisited do
			...
			if entry.depth < maxDepth then
				if #queue >= maxVisited then
					if #entry.node:GetChildren() > 0 then
						truncated = true
					end
				else
					for _, child in ipairs(sortedChildren(entry.node)) do
						if #queue >= maxVisited then
							truncated = true
							break
						end
