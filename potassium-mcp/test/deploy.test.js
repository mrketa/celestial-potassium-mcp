...
test("deploy installs exactly the canonical bootstrap and autoexec with byte parity", async (t) => { const value = await fixture(t); const state = await deploy(options(value)); assert.equal(state.schema, 2); assert.deepEqual(state.files.map(({ name }) => name), ["bootstrap", "autoexec"]); for (const file of state.files) { assert.equal(await readFile(file.source, "utf8"), await readFile(file.target, "utf8")); assert.equal(file.byteParity, true); } });
