...
import { formatToolResult, loadConfig } from "../src/server.js";
...
test("returns a compact actionable error for oversized tool results", () => {
  ...
  assert.equal(result.content[0].text, "Result too large; request less data");
...
test("completes MCP initialization with the bounded public tool set", async (t) => {
  ...
  assert.deepEqual(disconnected.content, [{ type: "text", text: "Potassium is not connected" }]);
});
